...
class TwoStepTranslator(LLM_API_Translator):
    ...
    params["system_prompt"][
        ...
        "You are a professional manga/comic localization editor. The user message "
        "contains JSON input items with id, source, and draft_translation produced by "
        "Google Translate or DeepL. Use the machine draft as a starting point, compare "
        "it against its original source, and return a more natural target-language "
        "translation suitable for dialogue and speech bubbles. Correct meaning, missing "
    ...
    params["reflection prompt"]["value"] = (
        "Perform a final localization review of the proposed translations. The original "
        "translation task below contains JSON input items with id, source, and the "
        "Google Translate or DeepL draft_translation; the draft translation JSON below "
        "contains the LLM proposal. Compare the proposal directly to the source while "
        "using the machine draft only as a reference. Fix mistranslations, omissions, "
