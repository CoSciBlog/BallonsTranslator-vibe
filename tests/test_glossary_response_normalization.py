...
class GlossaryResponseNormalizationTest(unittest.TestCase):
    ...
    def test_malformed_truncated_glossary_json_recovers_complete_entries(self):
        raw_json = (
            '{"entries":[{"source":"Tomori","target":"Tomori","category":"name"},'
            '{"source":"Eden","target":"Eden","category":"place"},'
            '{"source":"Broken","target:":333333333333333333'
        )
        logger = FakeLogger()

        data = LLM_API_Translator._loads_json_with_list_recovery(raw_json, "entries", logger)
        response = GlossaryResponse.model_validate(
            LLM_API_Translator._normalize_glossary_response_data(data)
        )

        self.assertEqual(len(response.entries), 2)
        self.assertEqual(response.entries[0].category, "character")
        self.assertEqual(response.entries[1].category, "place")
        self.assertIn(
            "Recovered 2 complete entries item(s) from malformed or truncated JSON response.",
            logger.warnings,
        )
