...
- Pipeline History now resizes visible columns proportionally with the window, gives wider module/model fields more space, redistributes hidden-column space, and elides long history paths without forcing an oversized window.
...
- Comic archive import and export:
  ...
  - imports a whole folder of `.cbr`, `.cbz`, `.zip`, `.pdf`, and nested image files through `Open -> Import Folder` or drag-and-drop
...
You can open `.cbz`, `.cbr`, `.zip`, and `.pdf` files through `Open -> Open Comic Archive/PDF`, by dropping a source file on the canvas, from recent projects, or by passing a single source path to `launch.py --proj-dir`. The file dialog and drag-and-drop also accept multiple PDFs, archives, or image files in one import.

Archives are extracted and PDFs are rendered into a regular image project folder next to the source file. For example, `Series.cbz` or `Series.pdf` becomes `Series/`; a multi-source selection is combined into one folder such as `Chapter 01_import/`. Nested image files and PDF pages are flattened into ordered page files such as `0001_page.png`, and import metadata is written to `archive_import.json`.
