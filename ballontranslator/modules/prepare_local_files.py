...
from ballontranslator.utils.download_util import DownloadCancelled, check_local_file, download_and_check_files
...
def iter_downloadable_modules():
    for category, registry in _registry_labels():
        for module_key, module_class in registry.module_dict.items():
            spec = _spec_from_module(module_class)
            if spec.download_file_list is None:
                continue
...
def module_download_status(module_spec_or_class, verify_hash: bool = False) -> Tuple[bool, List[str]]:
    missing = []
    spec = _spec_from_module(module_spec_or_class)
    for download_kwargs in spec.download_file_list or []:
...
def download_module_files(module_spec_or_class, progress_callback=None, cancel_event=None) -> bool:
    return ensure_module_files(module_spec_or_class, progress_callback, cancel_event)


def download_and_check_module_files(module_class_list: List = None):
    if module_class_list is None:
        module_class_list = []
        for category, module_key, module_class in iter_downloadable_modules():
            if (category, module_key) in OPTIONAL_STARTUP_DOWNLOADS:
                LOGGER.info(
                    f'Skipping optional startup model download for {category}/{module_key}. '
                    'Use Tools -> Model Downloads to fetch it.'
                )
                continue
    ...
    for module_class in module_class_list:
        spec = _spec_from_module(module_class)
        if spec.download_file_on_load or spec.download_file_list is None:
            continue
        all_successful = download_module_files(module_class)
        if all_successful:
            continue
        LOGGER.error(f'Please save these files manually to sepcified path and restart the application, otherwise {spec.key} will be unavailable.')
